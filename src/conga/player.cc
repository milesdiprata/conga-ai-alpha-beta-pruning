...
Player::Player(const Board::StoneType stone_type)
    : stone_type_(stone_type),
      opponent_stone_type_(stone_type == Board::StoneType::kBlack ? Board::StoneType::kWhite
...
void Player::MakeMove(Board& board, const Move& move) const {
  if (!ValidMove(board, move)) {
    throw invalid_argument("Invalid action " + to_string(move.action) + " for " +
                           to_string(stone_type_) + " from position " + to_string(move.point) +
                           "!");
  }

  for (int i = 1; i <= 3; ++i) {
    auto curr_point = move.point + (i * Board::kActionDirections.at(move.action));
    if (board.At(move.point).num_stones > 0 && board.HasPoint(curr_point) &&
      ...
      auto next_point = move.point + ((i + 1) * Board::kActionDirections.at(move.action));
      if (i == 3 || board.At(move.point).num_stones < i || !board.HasPoint(next_point) ||
...
const bool Player::Lost(const Board& board) const { return ValidMoves(board, stone_type_).empty(); }

const bool Player::ValidMove(const Board& board, const Move& move) {
  if (!board.HasPoint(move.point) || board.At(move.point).stone_type == Board::StoneType::kNone ||
      board.At(move.point).num_stones <= 0 || move.action == Board::Action::kNone) {
    return false;
  }

  auto opponent_stone_type = board.At(move.point).stone_type == Board::StoneType::kBlack
                                 ? Board::StoneType::kWhite
                                 : Board::StoneType::kBlack;

  auto next_point = move.point + Board::kActionDirections.at(move.action);
  if (!board.HasPoint(next_point) || board.At(next_point).stone_type == opponent_stone_type) {
    return false;
  }
...
const vector<Board::Action> Player::ValidActions(const Board& board, const Board::Point& point) {
...
const vector<Player::Move> Player::ValidMoves(const Board& board,
